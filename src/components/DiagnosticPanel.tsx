...
export const DiagnosticPanel: React.FC<DiagnosticPanelProps> = ({
  ...
  const handleForceReboot = () => {
    ...
    const rebootSequence: { text: string; type: 'normal' | 'warning' | 'error'; pct: number; phase: string }[] = [
  ...
  return (
    <div
      ...
      className="w-full chamfer-hud bg-[#121316]/95 border-2 border-[#1a4744] shadow-[0_0_40px_rgba(26,71,68,0.4)] relative overflow-hidden backdrop-blur-md select-none font-mono"
    >
      {/* Decorative Corner Reticles */}
      <div className="absolute top-2 left-2 text-[#1a4744]/70 text-[9px] pointer-events-none select-none">
        ┌── [DIAG_V4.2] ───────────────────────
      </div>
      <div className="absolute top-2 right-2 text-[#1a4744]/70 text-[9px] pointer-events-none select-none">
        ───────────────────── [SYS_STABLE: NO] ┐
      </div>
      <div className="absolute bottom-2 right-2 text-[#1a4744]/70 text-[9px] pointer-events-none select-none">
        [CONTAINER: AWS_EKS_FAILOVER] ┘
      </div>

      {/* Panel Top Header */}
      <div className="px-5 pt-7 pb-4 md:px-8 border-b border-[#1a4744]/50 flex flex-wrap items-center justify-between gap-4">
        <div className="space-y-1.5 max-w-2xl">
          <div className="flex items-center gap-2.5">
            <span className="w-2.5 h-2.5 bg-[#ff2a6d] shadow-[0_0_8px_#ff2a6d]"></span>
        ...
        <div className="chamfer-btn bg-[#0a0a0c] border border-[#e5c158] px-3.5 py-1.5 flex flex-col items-end shadow-[0_0_12px_rgba(229,193,88,0.2)]">
          <span className="text-[9px] text-gray-400 uppercase tracking-widest">
            CURRENT STATE
          </span>
          <span className="text-[#e5c158] font-bold text-xs uppercase flex items-center gap-1.5">
      ...
      <div className="p-5 md:p-8 space-y-6">
        ...
        <div className="space-y-2 bg-[#0a0a0c]/80 p-4 border border-[#2c3539] chamfer-card">
          <div className="flex flex-wrap items-center justify-between text-xs gap-2">
            <div className="flex items-center gap-2">
              <span className="text-gray-400 uppercase tracking-wider text-[11px]">
                RECONSTRUCTION LIFEBAR:
              </span>
              <span className="text-[#ff2a6d] font-bold text-xs">
                {phaseText}
              </span>
            </div>
            ...
            <div className="text-xs flex items-center gap-2">
              ...
              <span className="text-[#ff2a6d] font-bold text-sm glitch-active">
                {progressPercentage.toFixed(2)}%
              </span>
              <span className="text-[10px] text-[#e5c158] bg-[#e5c158]/10 border border-[#e5c158]/50 px-1.5 py-0.5 font-bold">
                [HANG_DETECTED]
              </span>
            </div>
          </div>
          ...
          <div className="w-full h-5 bg-[#0a0a0c] border border-[#1a4744]/80 p-0.5 relative overflow-hidden flex items-center shadow-inner">
            {/* Progress Fill Bar */}
            <div
              className="h-full bg-gradient-to-r from-[#1a4744] via-[#ff2a6d] to-[#ff2a6d] transition-all duration-300 relative shadow-[0_0_12px_rgba(255,42,109,0.7)]"
            ...
            <div className="absolute right-1 top-0 bottom-0 w-1 bg-[#e5c158] animate-pulse"></div>
          </div>
        ...
        <div className="space-y-2">
          ...
          <div
            ...
            className="bg-[#0a0a0c]/95 border border-[#1a4744]/80 p-4 text-xs sm:text-sm h-48 sm:h-52 overflow-y-auto space-y-1.5 shadow-inner chamfer-card focus:outline-none"
            tabIndex={0}
          >
            {logs.map((log) => {
              if (log.type === 'error') {
                return (
                  <div key={log.id} className="text-[#ff2a6d] font-bold flex items-center">
                    <span>{log.text}</span>
                    <span className="animate-blink font-bold text-[#ff2a6d] ml-0.5">_</span>
                  </div>
                );
              }
              if (log.type === 'warning') {
                return (
                  <div key={log.id} className="text-[#e5c158]">
                    {log.text}
                  </div>
                );
              }
        ...
        <div className="pt-2 flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-[#2c3539]">
          <div className="text-[11px] text-gray-400 text-center sm:text-left">
            ...
            <span>
              Attempts logged:{' '}
              <span className="text-[#ff2a6d] font-bold">{rebootAttempts}</span>. Hard fails
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto">
            ...
            <button
              ...
              className="flex-1 sm:flex-none text-center px-4 py-2.5 border border-[#2c3539] hover:border-gray-400 text-gray-400 hover:text-white bg-[#121316] text-xs font-bold uppercase chamfer-btn transition-colors cursor-pointer disabled:opacity-50"
            >
            ...
            <button
              ...
              className="flex-1 sm:flex-none px-6 py-2.5 bg-[#ff2a6d] hover:bg-[#ff2a6d]/90 text-black text-xs sm:text-sm font-bold tracking-wider uppercase chamfer-btn shadow-[0_0_25px_rgba(255,42,109,0.5)] transition-all active:scale-95 flex items-center justify-center gap-2 cursor-pointer disabled:opacity-80"
            >
      ...
      <div className="bg-[#121316] px-5 py-3 border-t border-[#1a4744]/50 grid grid-cols-2 sm:grid-cols-4 gap-2 text-[10px] text-gray-400">
        <div>
        ...
        <div>
          ...
          <span className="text-[#ff2a6d] font-bold">14.8%</span>
        </div>
        <div>
        ...
        <div className="text-right sm:text-left">
          ...
          <span className="text-[#e5c158] font-bold">RESTRICTED</span>
        </div>
      </div>
    </div>
  );
