...
import { playHoverPip, playTone } from '~/src/utils/audio';
...
const toggleAudio = () => {
  emit('toggle-audio');
  if (props.isMuted) playTone(880, 'triangle', 0.08, 0.08);
};
</script>
  ...
  >
    <div class="flex items-center gap-4 md:gap-6">
      <div class="flex items-center gap-2.5">
        <span class="relative flex h-2.5 w-2.5">
          <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#ff2a6d] opacity-75"></span>
      ...
      <div class="hidden md:flex items-center gap-2.5 border-l border-[#2c3539] pl-4">
        ...
        <div class="flex items-center gap-1">
          ...
          <div class="w-2.5 h-4 bg-[#ff2a6d] chamfer-pill shadow-[0_0_8px_rgba(255,42,109,0.5)]"></div>
          <div class="w-2.5 h-4 bg-[#ff2a6d]/30 chamfer-pill"></div>
          <div class="w-2.5 h-4 bg-[#ff2a6d]/20 chamfer-pill"></div>
    ...
    <div class="flex items-center gap-3 md:gap-6 text-[11px]">
      ...
      <button
        ...
        class="px-2.5 py-1 border border-[#2c3539] hover:border-[#ff2a6d] hover:text-[#ff2a6d] bg-[#0a0a0c]/80 text-gray-300 transition-colors uppercase text-[10px] flex items-center gap-1.5 cursor-pointer chamfer-btn"
        title="Toggle UI Web Audio Synthesizer (Key: M)"
      >
        <VolumeX v-if="isMuted" class="w-3.5 h-3.5 text-gray-400" />
        <Volume2 v-else class="w-3.5 h-3.5 text-[#ff2a6d] animate-pulse" />
